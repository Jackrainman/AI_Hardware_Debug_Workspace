...
建议进入一个受控的 UI 改造小阶段，但不能直接大改 UI。UI 改造主目标不是做新 dashboard，而是把现有 ProbeFlash 主流程重新整理成更清楚的现场问题闭环：当前项目、快速建卡、问题详情、排查时间线、Knowledge Assist、结案和归档复盘各有明确位置。当前已完成 `UI-01-INFORMATION-ARCHITECTURE-REVIEW`、`UI-GATE-01-MANUAL-VISUAL-DIRECTION` 和 `TECH-07-APP-TSX-MINIMAL-SPLIT`，用户已认可 TECH-07 拆分结果。下一步不是直接进入 shell、issue flow 或 Knowledge Assist 视觉实现，而是先执行 `UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT` 行为保持模块化拆分；完成后停在 `UI-GATE-03-MANUAL-RUN-CHECK-BEFORE-RELAYOUT`，等用户人工确认现有功能能正常跑，再进入后续 UI 重排。

当前交接更新：`UI-GATE-01-MANUAL-VISUAL-DIRECTION` 已在 2026-04-30 获得用户确认并落盘；`TECH-07-APP-TSX-MINIMAL-SPLIT` 已完成且用户已认可拆分结果。下一 repo-local 任务只能是 `UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT`：先做行为保持模块化拆分，不做 UI 重排；完成后必须停在 `UI-GATE-03-MANUAL-RUN-CHECK-BEFORE-RELAYOUT`，等待用户人工检查能否正常跑。
...
唯一推荐的下一 UI 任务是 `UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT`。

判断依据：信息架构、视觉方向和 TECH-07 支撑拆分已经完成，但 `App.tsx` 仍承载大量具体 UI 组件。如果直接做三栏布局、QuickIssue landing 或 Knowledge Assist 合并，会把“组件搬家”和“视觉重排”混在同一轮，回归面过大。`UI-MOD-01` 只做行为保持组件拆分，不能推荐“直接重写整个 UI”。

后续顺序：`UI-MOD-01` 完成后必须停在 `UI-GATE-03-MANUAL-RUN-CHECK-BEFORE-RELAYOUT`；用户人工确认能正常跑后，才允许选择 UI-02 / UI-03 / UI-04 等真正 UI 重排任务。`TECH-08` / `TECH-09` / `TECH-10` 等非 UI 支撑型重构应后置到具体 server / storage 任务命中时。
...
| UI-MOD-01 | night-safe | 下一轮唯一可自动认领任务，只做行为保持模块化拆分。 |
| UI-GATE-03 | manual-blocked | 模块化拆分后必须人工确认页面能正常跑，不能夜跑越过。 |
...
- `DEP-01-RELEASE-USER-DIR-DEPLOY-VERIFY` 的 P0 白天主线仍 blocked；但下一轮默认只认领 `UI-MOD-01`，除非用户明确推翻本轮模块化优先约束并重新授权服务器部署。
- 推荐下一 UI 任务是 `UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT`；它只做行为保持模块化拆分，不实际改 UI 层级或视觉。
- B 组功能完成后，UI 大问题优先级高于 broad refactor；当前执行顺序为 `UI-GATE-01-MANUAL-VISUAL-DIRECTION` -> `TECH-07-APP-TSX-MINIMAL-SPLIT` -> `UI-GATE-02` 用户认可 -> `UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT` -> `UI-GATE-03-MANUAL-RUN-CHECK-BEFORE-RELAYOUT` -> 后续 UI 重排。
...
`TECH-07` 拆分结果已被用户认可，但 `UI-GATE-02` 不直接放行 UI 重排。下一步必须先执行 `UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT`，只做行为保持模块化拆分；完成后停在 `UI-GATE-03-MANUAL-RUN-CHECK-BEFORE-RELAYOUT`。用户人工确认能正常跑之后，第一轮 UI 修改才优先做 shell / workspace 状态 / Knowledge Assist 主次关系；不得在第一轮做 closeout 重排、archive drawer 重做、真实 AI、RAG、Code context、Electron / fs / IPC 或 server / schema / repository / HTTP API 改动。
