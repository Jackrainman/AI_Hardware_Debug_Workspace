...
## 1. 一句话状态
ProbeFlash 已具备本地 HTTP + SQLite + release 可部署基座、基础知识检索、轻量相似问题提示、历史问题人工关联和复发提示；Search / KB 小阶段已收尾，当前白天主线仍卡在真实服务器用户目录部署确认，无服务器授权时下一 night-safe 是 `UI-REDESIGN-STAGE-BRIEF`。
...
## 4. 当前夜跑候选
- `UI-REDESIGN-STAGE-BRIEF`：只写下一阶段 UI 改造 brief，不改 App.tsx / CSS / 组件。
...
- `SEARCH-01-BASIC-FULL-TEXT-SEARCH`：SQLite 主链路支持基础全文搜索。
...
## 7. 用户下一步
- 今天完全不想动：让 AI 夜跑 `UI-REDESIGN-STAGE-BRIEF` 这类 repo-local 任务，不碰服务器和真实 AI。
