...
## 1. 一句话状态
ProbeFlash 已具备本地 HTTP + SQLite + release 可部署基座和基础知识检索能力，当前白天主线仍卡在真实服务器用户目录部署确认；无服务器授权时下一 night-safe 是 `SEARCH-07-SIMILAR-ISSUES-LITE`。
...
## 4. 当前夜跑候选
- `SEARCH-07-SIMILAR-ISSUES-LITE`：只做 repo-local 轻量相似度排序和 verify，不接 embedding / RAG / 真实 AI。
...
- `CORE-07-ARCHIVE-FILTERS`：扩展本地归档浏览筛选，不触碰部署、AI 或权限。
...
- `74a3cbc`：`DATA-02-JSON-EXPORT-HARDEN`：JSON export 字段与脱敏边界更稳。
...
## 7. 用户下一步
- 今天完全不想动：让 AI 夜跑 `SEARCH-07-SIMILAR-ISSUES-LITE` 这类 repo-local 任务，不碰服务器和真实 AI。
