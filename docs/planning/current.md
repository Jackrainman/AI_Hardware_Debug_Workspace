...
- WSL 本地已验证 release 包可下载、解压、运行：web dist 可被临时静态服务器托管，`/api` 可代理到 `127.0.0.1:4100`，server release 可启动，`/api/health` 返回 sqlite ready；停掉 backend 后 proxy 返回 `502 proxy_error`，没有 silent fallback。
...
- `apps/server/deploy/*` 当前应表达 release tarball first：GitHub Release 固定资产 + SHA256 校验 + 用户目录解压 + `current` symlink + `shared` 持久目录；`git pull` 只可作为开发 / 调试方式，不是正式部署方式。
...
- **S3-SERVER-RELEASE-USER-DIR-DEPLOY-VERIFY**
  ...
  - 验证要求：校验 `SHA256SUMS.txt`；确认 `current -> releases/v0.2.0`；确认 server release 用独立 Node runtime 启动；`curl http://127.0.0.1:4100/api/health`；`curl http://192.168.2.2:4100/api/health`；创建 workspace / issue；停止重启后读回；确认 `/home/hurricane/probeflash/shared/data/probeflash.sqlite` 被使用；确认 filebrowser:80 仍正常。
...
- **S3-SERVER-RELEASE-STATIC-WEB-SERVE-PLAN**
  - 状态：`pending_repo_local_plan`；night-safe，仅限仓库内文档 / 脚本计划，不启动真实服务器。
  - 选择理由：需要明确 web dist 如何服务，以及 `/api` 如何代理到 backend；当前优先独立 Node 静态代理方案，不碰 80，不做反向代理。
...
2. `S3-SERVER-RELEASE-STATIC-WEB-SERVE-PLAN`：pending_repo_local_plan，夜跑可选，只能更新 deploy docs / 本地计划，不碰真实服务器。
3. `S3-SERVER-RELEASE-UPDATE-FLOW`：pending_after_release_user_dir_verify；定义 v0.2.1 / v0.3.0 下载新 release、解压到 `releases/vX.Y.Z`、切 `current`、restart、health check、rollback。
4. `S3-SERVER-SYSTEMD-AUTOSTART-PREP`：blocked_by_external_dependency_after_release_user_dir_verify，白天主线后续，不执行 systemctl。
5. `S3-SERVER-SYSTEMD-AUTOSTART-VERIFY`：blocked_by_user_confirmation，涉及 sudo / systemd。
6. `TECH-DEBT-CLOSEOUT-ATOMICITY-RECOVERY`：pending_night_safe_candidate，夜跑可选，repo-local 临时 DB / fixture。
7. `TECH-DEBT-APP-SPLIT-MINIMAL`：pending_after_closeout_recovery，夜跑可选，必须小步且自动验证。
8. `TECH-DEBT-VERIFY-HELPERS`：pending_after_app_split，夜跑可选，repo-local。
9. `TECH-DEBT-VERIFY-TMP-CLEANUP`：pending_after_verify_helpers，夜跑可选，只能清理 repo-local 临时路径。
...
- 若用户没有服务器时间：不要执行真实部署；可选择第一个 repo-local、可自动验证、可回滚的 night-safe 任务。当前 release 相关候选是 `S3-SERVER-RELEASE-STATIC-WEB-SERVE-PLAN`；技术债候选是 `TECH-DEBT-CLOSEOUT-ATOMICITY-RECOVERY`。
...
- 本轮 release-based deployment planning 验证要求：`git diff --check`、`python3 -m json.tool .agent-state/handoff.json >/dev/null`、`cd apps/desktop && npm run verify:handoff`、`cd apps/server && npm run verify:deploy-prep`、`git status --short`。
- 本轮只改 docs / planning / deploy materials / 静态校验，不改业务代码；因此不需要跑 typecheck / build / verify:all，但必须在 handoff / 汇报中如实说明。
