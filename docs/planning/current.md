...
- 阶段目标：以 v0.2.x 已完成的本地 HTTP + SQLite + release 可部署基座为起点，按 8 条产品主线推进；近期 P0 仍关注 **部署可用、数据安全、可观测**。B 组、`UI-GATE-01-MANUAL-VISUAL-DIRECTION`、`TECH-07-APP-TSX-MINIMAL-SPLIT` 与 `UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT` 均已完成；当前必须停在 `UI-GATE-03-MANUAL-RUN-CHECK-BEFORE-RELAYOUT`，等用户人工检查页面能否正常跑。AI 不得自动进入三栏布局、QuickIssue landing、Knowledge Assist 合并或 closeout 视觉重构。
- 路线图事实源：`docs/planning/product-roadmap.md`。
- 最近已完成：`UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT`，已将 `QuickIssueCreateBar`、`IssueIntakeForm`、`IssueCardListView`、Knowledge Assist 四个面板、`InvestigationAppendForm`、`InvestigationRecordListView`、`CloseoutForm` 与 `MainlineResultPanel` 从 `App.tsx` 抽到独立 UI 模块；未做视觉重设计，未改 render 顺序、用户文案、CSS 主视觉、业务数据流、schema、repository、HTTP API 或 server。

## 当前真实状态
- 已完成：本地 HTTP + SQLite 主链路、workspace 创建 / 切换、workspace UX improvements、recent issue reopen、closeout failure input preservation hints、AI-ready draft history、issue / record / closeout / archive / error-entry 主路径、basic full-text search、search filters、search tags、archive review page、similar issues lite、search result linking、recurrence prompt、search / KB verify fixture cleanup、UI redesign stage brief、UI information architecture review、App.tsx minimal split、UI pre-relayout component split、quick issue create、record timeline polish、closeout UX polish、`ErrorEntry.prevention` 非空修复、release tarball 部署规划、server 同端口服务 `dist` + `/api`、AI-ready prompt templates、rule-based closeout draft panel、server schema contract、HTTP feedback contract、restore dry-run、SQLite integrity check、JSON export hardening、partial closeout recovery verify、repair task generation、diagnostics bundle、night-run 安全规则、v0.2 历史文档归档、lightweight project status ledger、refactor necessity audit。
...
## 已完成 repo-local 自动认领任务（不能继续顺推 UI 重排）
- **UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT**
  ...
  - 当前状态：`completed`；repo-local / night-safe；已完成并进入人工检查门。
...
- **UI-MOD-01-PRE-RELAYOUT-COMPONENT-SPLIT**
  - 状态：`completed`；P1；repo-local / night-safe。
  - 选择理由：用户已确认 UI 层级需要全量重排，但要求先做模块化拆分；该拆分已完成，下一步只能人工检查。
- **UI-GATE-03-MANUAL-RUN-CHECK-BEFORE-RELAYOUT**
  - 状态：`current`；P1；day-only / manual smoke。
  - 选择理由：模块化拆分通过自动验证后，仍需用户人工确认页面能正常跑，才能进入真正 UI 重构。

## 下一步最小可执行动作
- 下一轮默认：停在 `UI-GATE-03-MANUAL-RUN-CHECK-BEFORE-RELAYOUT`；用户需要人工启动 / 浏览 ProbeFlash，确认现有页面能否正常跑。
- 无服务器授权或 repo-local 执行窗口：没有可自动顺推的 UI 任务；AI 必须停止，不能继续做三栏布局、QuickIssue landing、Knowledge Assist 合并、closeout 视觉重构或任何 UI implementation。
- `UI-MOD-01` 已完成：等待用户人工检查能否正常跑；不得自动顺推到三栏布局、QuickIssue landing、Knowledge Assist 合并或 closeout 视觉重构。
