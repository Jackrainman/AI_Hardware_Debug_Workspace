...
- **CORE-04-RECORD-TIMELINE-POLISH**
  ...
  - 选择理由：快速建卡已完成，下一步继续改善现场排查记录的可读性；不依赖服务器授权。
- **CORE-05-CLOSEOUT-UX-POLISH**
  - 状态：`night-safe`；P1；repo-local UI / closeout smoke。
  - 选择理由：时间线可读性收紧后，继续降低结案漏填、误填风险；不接真实 AI。

## 下一步最小可执行动作
- 白天有用户参与：认领 `DEP-01-RELEASE-USER-DIR-DEPLOY-VERIFY`，执行前再次复述 SSH / release assets / 写入路径 / 临时进程 / 4100 授权边界。
- 无服务器授权或夜跑：不要部署；下一轮重新读取事实源后，优先从 pending queue / night-safe pool 认领 `CORE-04-RECORD-TIMELINE-POLISH`。
