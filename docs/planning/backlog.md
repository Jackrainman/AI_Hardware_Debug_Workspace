...
- 直接输入边界：`apps/desktop/src/App.tsx`、`apps/desktop/src/storage/*`、相关 domain schema / intake / closeout 代码。
- 不做项：不接 HTTP、不写后端、不改服务器部署、不引入大状态方案、不全量重命名 `projectId`。
- 工程化验证：
  - `cd apps/desktop && npm run typecheck`
  - `cd apps/desktop && npm run build`
  - `cd apps/desktop && npm run verify:all`
  - `git diff --check`
  - 任务相关验证：证明 localStorage 读写已走 async port，且写结果不再是裸 `void`。
- 完成定义：async port 已落地；localStorage adapter 已接入；当前主流程写操作已返回统一结构化 result；下一任务 `S3-ARCH-CLOSEOUT-ORCHESTRATOR` 依赖解除。
- 后继依赖：`S3-ARCH-CLOSEOUT-ORCHESTRATOR`。
