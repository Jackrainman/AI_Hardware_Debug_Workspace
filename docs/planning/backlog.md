...
| 19 | UI-GATE-05-MANUAL-COPY-TRIM-REVIEW | current/day-only | P1 |
| 20 | TECH-09-SERVER-ROUTE-SPLIT | night-safe | P2 |
...
- 当前没有可自动顺推的 UI 相关 repo-local 任务；`UI-POLISH-02-COPY-TRIM` 已完成。
- 当前必须停在 `UI-GATE-05-MANUAL-COPY-TRIM-REVIEW`，不能夜跑越过人工检查门。
...
- 不在 `UI-GATE-05-MANUAL-COPY-TRIM-REVIEW` 人工 review / smoke 通过前执行下一轮 UI polish；不把文案瘦身扩展成全量重写。
