...
| 17 | UI-GATE-04-MANUAL-RELAYOUT-REVIEW | current/day-only | P1 |
| 18 | TECH-09-SERVER-ROUTE-SPLIT | night-safe | P2 |
...
- 当前没有可自动顺推的 UI 相关 repo-local 任务；`UI-RELAYOUT-01-WORKBENCH-FIRST-PASS` 已完成。
- 当前必须停在 `UI-GATE-04-MANUAL-RELAYOUT-REVIEW`，不能夜跑越过人工检查门。
...
- 不在 `UI-GATE-04-MANUAL-RELAYOUT-REVIEW` 人工 review / smoke 通过前执行第二轮 UI polish；不把第一轮工作台重排扩展成全量重写。
