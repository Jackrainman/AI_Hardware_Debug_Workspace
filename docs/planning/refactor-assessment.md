...
当前不建议在 `DEP-01-RELEASE-USER-DIR-DEPLOY-VERIFY` 或 `SEARCH-07-SIMILAR-ISSUES-LITE` 前插入强制重构任务。代码库确实存在大文件、重复搜索 / 标签归一化、verify fixture 重复和 server 数据库职责过重，但这些问题目前没有阻塞本地 HTTP + SQLite 主链路，也不是 DEP-01 服务器用户目录部署验证的前置条件。对 SEARCH-07 来说，重复逻辑会增加实现时的维护成本，但边界仍可控，适合在 SEARCH-07 内最小增量处理，而不是先做独立重构。

推荐下一步唯一可自动推进任务是 `SEARCH-07-SIMILAR-ISSUES-LITE`；如果用户白天确认服务器边界，`DEP-01-RELEASE-USER-DIR-DEPLOY-VERIFY` 仍是 P0 白天主线且不需要等待重构。
...
| --- | --- | --- | --- |
| App shell coupling | `App.tsx` 2516 LOC；`IssuePane` 维护 selected issue、record list、closeout summary；`App` 维护 workspace repository、archive index、storage feedback。 | Medium for UI feature velocity; Low for DEP-01. | Do not split before DEP-01 / SEARCH-07. Future `TECH-07-APP-TSX-MINIMAL-SPLIT` only when UI task already touches same area. |
| HTTP repository concentration | `http-storage-repository.ts` 891 LOC；list methods repeat envelope parse + invalid bucket normalization for workspaces/issues/records/archives/error-entries. | Medium for future API additions and SEARCH-07 endpoint work. | Defer. If SEARCH-07 adds an endpoint, add the smallest method and avoid unrelated adapter restructuring. |
| Dual search implementations | localStorage search in `storage-repository.ts` lines 264-420; server SQLite search in `database.mjs` lines 731-875; HTTP adapter response schema in `http-storage-repository.ts` lines 95-125. | Medium for SEARCH-07 consistency. | Not blocking. Implement SEARCH-07 with explicit fixtures covering server + desktop instead of pre-refactoring search. |
| Repeated tag normalization | `App.tsx` lines 308-319; `storage-repository.ts` lines 137-162; `http-storage-repository.ts` lines 196-207; `database.mjs` lines 95-119 and 424-440. | Medium for tags / similar issues. | Defer. If a mismatch appears during SEARCH-07, use a tiny `domain/tags` helper in desktop and keep server behavior covered by verify. |
...
| --- | --- | --- | --- | --- |
| `TECH-07-APP-TSX-MINIMAL-SPLIT` | `App.tsx` 2516 LOC; high-frequency recent changes in search/archive/core UX commits. | Medium | High | Defer. Do not do before DEP-01 / SEARCH-07. |
...
No candidate simultaneously satisfies “blocks S3 / DEP-01 / SEARCH-07” and “small, safe, clearly verifiable” strongly enough to justify inserting a refactor gate.

### Defer

- `TECH-07-APP-TSX-MINIMAL-SPLIT`: do only when a UI task already touches the same components; forbidden scope remains full App rewrite, visual redesign, storage behavior changes.
...
Recommended unique repo-local next task: `SEARCH-07-SIMILAR-ISSUES-LITE`.

Rationale: DEP-01 remains the P0 daytime task, but it is blocked by user confirmation of SSH, release asset transfer, `/home/hurricane/probeflash` write permission, temporary process start and port 4100 boundary. With no server authorization in this task, SEARCH-07 is the first pending night-safe task and is not blocked by any prerequisite refactor.

If a small refactor is later forced by SEARCH-07 evidence, the minimum safe boundary is: extract only duplicated desktop-side tag normalization or verify fixture helpers touched by SEARCH-07, keep server API contracts unchanged, keep `App.tsx` layout unchanged, keep localStorage compatibility, and verify with `typecheck`, `build`, `verify:handoff`, relevant search verifies and `verify:all`.
