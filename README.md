...
当前 D1 产品壳与浏览器 smoke 已完成，项目进入 **S3：存储迁移与服务器化**。下一阶段不优先做 AI / RAG / 权限系统 / Electron，而是把当前 `window.localStorage` 演示版升级为“战队局域网可访问 + 服务器长期存储”的版本。

S3 目标访问形态：同一 WiFi 下通过类似 `http://hurricane-server.local:<port>/` 的地址访问；当前尚未写后端代码，仍需先完成服务器环境、端口、域名/入口方式、权限和部署条件盘点。
...
| 🗂 知识库积累 | 让调试记录从临时救火变成可复用资产 | 当前为 localStorage 原型，S3 迁移服务器长期存储 |
...
| 原型存储 | `window.localStorage`（D1 已验证，S3 待迁移） |
| S3 后端方向 | Node HTTP API（待 S3-BACKEND-SCAFFOLD 确认） |
| S3 长期存储方向 | SQLite / 服务端数据目录（待 S3-SQLITE-STORAGE 落地） |
| 局域网访问方向 | `http://hurricane-server.local:<port>/` 或服务器 IP + 端口 |
| 结构校验 | zod schema |
...
S3 的交付目标不是继续停留在这个本地演示地址，而是完成局域网入口与服务端长期存储迁移。预期形态是同一 WiFi 下访问：

```text
http://hurricane-server.local:<port>/
```

本仓库当前尚未实现后端服务；S3 第一入口任务是 `S3-SERVER-INVENTORY`，先确认服务器环境、端口、域名/入口方式、权限和部署条件。
...
- D1 产品壳与浏览器 smoke 已完成，但当前真实运行仍是浏览器 SPA + `window.localStorage`。
- S3 目标是局域网共享 + 服务器长期存储，后端 API、SQLite 数据库、部署脚本和多设备 smoke 尚未实现。
- 当前不把 localStorage 数据包装成服务器持久化；刷新保留不等于跨设备共享。
...
- `S3-SERVER-INVENTORY`：先盘点服务器环境、端口、域名/入口方式、权限与部署条件。
- `S3-BACKEND-SCAFFOLD`：建立最小后端 API 与健康检查，不接 AI。
- `S3-SQLITE-STORAGE`：把 IssueCard / InvestigationRecord / ErrorEntry / ArchiveDocument 迁移到服务端 SQLite。
- `S3-FRONTEND-STORAGE-ADAPTER`：让前端从 localStorage 演示存储切到 HTTP storage adapter，并保留可回退边界。
- `S3-LAN-DEPLOY`：提供局域网可访问部署方式与启动说明。
- `S3-MULTI-DEVICE-SMOKE`：验证两台设备访问同一服务器看到同一数据。
