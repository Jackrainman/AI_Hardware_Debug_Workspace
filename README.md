...
S3 目标访问形态：同一 WiFi 下通过服务器 IP + 独立端口访问；本地 HTTP + SQLite release 测试已通过，真实服务器 release tarball / LAN 部署验证尚未完成。后续优先从 GitHub Release 压缩包部署，不把服务器当 `git pull` 开发态 checkout。
...
S3 的交付目标不是停留在本地开发地址，而是在真实服务器上用 GitHub Release 压缩包完成独立 runtime、独立端口与 SQLite 持久化验证；systemd 自启是 no-sudo release 验证通过后的后续步骤。该服务器部署验证尚未完成。
...
- 下一项：服务器安全部署验证，优先使用 GitHub Release tarball、`SHA256SUMS.txt` 校验、`/home/hurricane/probeflash/releases/vX.Y.Z`、`current` symlink 与 `shared/data` 持久目录；不以服务器 `git pull` 作为主部署方式。
